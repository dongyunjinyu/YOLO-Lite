...
from yololite.utils.checks import check_file, check_imgsz, print_args
...
class DetectionTrainer:
    ...
    def _setup_train(self):
        ...
        for k, v in self.model.named_parameters():
            ...
            if any(x in k for x in freeze_layer_names):
                ...
                LOGGER.info(
                    ...
                    "See yololite.engine.trainer for customization of frozen layers."
                )
                v.requires_grad = True

        # Check AMP
        self.amp = torch.tensor(self.args.amp).to(self.device)  # True or False
        if self.amp:  # Single-GPU
        ...
        self.test_loader = self.get_dataloader(
            self.testset, batch_size=batch_size * 2, mode="val"
        )
    ...
    def setup_model(self):
        ...
        self.model = self.get_model(cfg=cfg, weights=weights, verbose=True)  # calls Model(cfg, weights)
        return ckpt
    ...
    def check_resume(self, overrides):
        ...
        if resume:
            try:
                ...
                raise FileNotFoundError("Resume checkpoint not found.") from e
        self.resume = resume
