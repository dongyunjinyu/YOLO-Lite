...
from yololite.data.utils import polygons2masks, polygons2masks_overlap
...
from yololite.utils.ops import segment2box, xyxyxyxy2xywhr
...
class Mosaic(BaseMixTransform):
    ...
    Examples:
        >>> from yololite.data.augment import Mosaic
...
class MixUp(BaseMixTransform):
    ...
    Examples:
        >>> from yololite.data.augment import MixUp
        >>> dataset = YourDataset(...)  # Your image dataset
...
class RandomHSV:
    ...
    Examples:
        >>> import numpy as np
        >>> from yololite.data.augment import RandomHSV
    ...
    def __call__(self, labels):
        ...
        Args:
            labels (Dict): A dictionary containing the following keys:
                ...
                'instances' (yololite.utils.instance.Instances): An object containing bounding boxes and
                    optionally keypoints.

        Returns:
            (Dict): The same dictionary with the flipped image and updated instances:
                'img' (numpy.ndarray): The flipped image.
                'instances' (yololite.utils.instance.Instances): Updated instances matching the flipped image.
...
class CopyPaste(BaseMixTransform):
    ...
    Examples:
        >>> from yololite.data.augment import CopyPaste
        >>> dataset = YourDataset(...)  # Your image dataset
