...
from yololite.utils import IS_COLAB, IS_KAGGLE, LOGGER, TryExcept, ops, plt_settings, threaded
from yololite.utils.checks import check_version, is_ascii
...
class Annotator:
    ...
    def __init__(self, im, line_width=None, font_size=None, font="Arial.ttf", pil=False, example="abc"):
        """Initialize the Annotator class with image and line width along with color palette for limbs."""
...
def plot_labels(boxes, cls, names=(), save_dir=Path(""), on_plot=None):
    ...
    import pandas  # scope for faster 'import yololite'
...
def save_one_box(xyxy, im, file=Path("im.jpg"), gain=1.02, pad=10, square=False, BGR=False, save=True):
    ...
    Example:
        ```python
        from yololite.utils.plotting import save_one_box
...
def plot_images(
    ...
    """
    Plot image grid with labels, bounding boxes.
...
def plot_results(file="path/to/results.csv", dir="", on_plot=None):
    """
    Plot training results from a results CSV file. Plots are saved as 'results.png' in the directory where the CSV is located.
    Args:
        ...
        on_plot (callable, optional): Callback function to be executed after plotting. Takes filename as an argument.
            Defaults to None.
    """
    import pandas as pd  # scope for faster 'import yololite'
...
def feature_visualization(x, module_type, stage, n=32, save_dir=Path("runs/detect/exp")):
    ...
    """
    for m in {"Detect"}:  # all model heads
        if m in module_type:
            return
