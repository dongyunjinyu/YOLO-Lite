...
def model_info_for_loggers(trainer):
    results = {"model/parameters": get_num_params(trainer.model), "model/GFLOPs": round(get_flops(trainer.model), 3),
               "model/speed_PyTorch(ms)": round(trainer.validator.speed["inference"], 3)}
    return results
...
def strip_optimizer(f: Union[str, Path] = "best.pt", s: str = "", updates: dict = None) -> dict:
    ...
    Example:
        ```python
        from pathlib import Path
        from yololite.utils.torch_utils import strip_optimizer
    ...
    Note:
        Use `yololite.nn.torch_safe_load` for missing modules with `x = torch_safe_load(f)[0]`
    """
    try:
...
def profile(input, ops, n=10, device=None, max_num_obj=0):
    ...
    Example:
        ```python
        from yololite.utils.torch_utils import profile
