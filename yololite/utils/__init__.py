...
HELP_MSG = """
    ...
    1. Install the yololite package:
...
class IterableSimpleNamespace(SimpleNamespace):
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(
            f"""
            '{name}' object has no attribute '{attr}'. This may be caused by a modified or out of date yololite
            'default.yaml' file.\nPlease update your code with 'pip install -U yololite' and if necessary replace
...
def set_logging(name="LOGGING_NAME", verbose=True):
    ...
    Examples:
        >>> set_logging(name="yololite", verbose=True)
        >>> logger = logging.getLogger("yololite")
...
class SettingsManager(JSONDict):
    ...
    def __init__(self, file=SETTINGS_FILE, version="0.0.6"):
        ...
        from yololite.utils.torch_utils import torch_distributed_zero_first
...
def vscode_msg(ext="yololite.yololite-snippets") -> str:
